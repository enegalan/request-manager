...
class RequestManager {
     ...
     * @returns {Object} The options for the current request
     */
    getOptions() {
        return this.options;
    }
     ...
     * @returns {Object} The manager options
     */
    getManagerOptions() {
    ...
    #_prepareFetchOptions(options, signal, additionalOptions = {}) {
        ...
        const customOptions = ['abortController', 'cancelToken', 'requestKey', 'verbose', 'noCancel'];
     ...
     * @param {boolean} options.verbose - If true, cancellation errors will include messages
     ...
     * @param {boolean} options.verbose - If true, cancellation errors will include messages. Overrides global verbose setting.
    ...
    axios(url, options = {}) {
        ...
        const requestId = this.#_generateRequestId(url, requestOptions.requestKey, requestOptions.noCancel);
        return this.#_request(requestId, axios.get(url, { cancelToken: cancelToken.token, ...requestOptions }), {
    ...
    cancel(requestId) {
        ...
        if (requestInfo.rejectWrapper) {
            const verbose = requestInfo.verbose !== undefined ? requestInfo.verbose : this.verbose;
            if (verbose) requestInfo.rejectWrapper(new Error('Request was cancelled'));
        }
